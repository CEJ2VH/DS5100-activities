...
# Student Info
    ...
    Name:  Sarah Hall
    Net ID: CEJ2VH
    File GitHub URL:https://github.com/CEJ2VH/DS5100-activities/blob/main/activities/M11-HW-2.Rmd
...
```{r}
# CODE HERE
  print(nrow(data %>% filter(data$V1=='I')))
```
...
```{r}
# CODE HERE
  print(nrow(data %>% filter(data$V1=='I'|data$V1=='M')))
...
gender <-table(data$V1)
gender.df<-data.frame(gender)
names(gender.df) <- c("Gender", "Freq")
t(gender.df)
...
data %>% 
  group_by(V1) %>% summarize(mean=mean(V2))
```
...
data %>% 
  dplyr::filter(V1=='M') %>% summarize(med=median(V2))
